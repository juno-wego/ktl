...
Nav2가 사용하는 지도 기준 로봇 위치는 `map → base_link` TF로 확인한다.

```bash
ros2 run tf2_ros tf2_echo map base_link
```

`Translation`의 `x`, `y`는 지도 원점 기준 위치(m)이고, `Rotation`의 RPY에서 `yaw`는
로봇이 바라보는 방향(rad)이다. 값은 계속 출력되며 `Ctrl+C`로 종료한다. TF를 찾지 못하면
AMCL 초기 위치, `/scan`, `map → odom → base_link` 연결을 확인한다.

AMCL이 계산한 위치와 추정 불확실성까지 한 번만 확인하려면 다음 명령을 사용한다.

```bash
ros2 topic echo /amcl_pose --once
```

`pose.pose.position`은 지도 기준 위치, `pose.pose.orientation`은 quaternion 방향값이다.
`pose.covariance`는 추정 불확실성을 나타내며 값이 크거나 위치가 계속 튀면 LiDAR 관측과
지도의 벽이 맞는지 확인한다. 로봇이 정지해 있어 새 AMCL 결과가 발행되지 않으면 명령이
기다릴 수 있다.
...
영역이 되도록 비용을 낮춘다. 현재 로봇 footprint는 반경 `0.28 m`이고, 장애물에서 비용을
퍼뜨리는 inflation 반경은 로컬 `0.45 m`, 전역 `0.50 m`다. 이 값은 로봇 반경에 더하는
여유 거리가 아니라 장애물 기준 전체 반경이다.
...
이동 명령은 한 번만 보내는 것이 아니라 주기적으로 보내야 한다. 브리지는 마지막 `/cmd_vel`이
0.5초 동안 갱신되지 않으면 속도가 모두 0인 `Move` 명령을 한 번 보낸다. 이는 통신 단절 시
로봇이 계속 움직이지 않도록 하는 보호 동작이다.

## Waypoint 활용

waypoint follower를 사용할 때 여러 지점은 `FollowWaypoints` 액션으로 전달한다. 이 노드는
각 waypoint를 `NavigateToPose`로 순차 실행하고, 각 지점에서 task executor를 수행한다.
모든 waypoint는 `map` 프레임을 사용하며 각 지점의 방향을 지정할 수 있다.


현재 waypoint follower는 도착 후 200ms 대기하고, 한 waypoint에 실패하면 그 지점을
기록한 뒤 다음 waypoint를 계속 시도하도록 `stop_on_failure: false`로 설정되어 있다.
