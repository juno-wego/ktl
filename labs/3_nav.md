...
현재 Navigation은 전역 플래너 NavFn과 로컬 컨트롤러 MPPI를 사용한다. 저장된 지도에서 AMCL로 위치를 찾고, LiDAR 기반 costmap으로 장애물을 피하며 `/cmd_vel`을 생성한다.
...
| MPPI | 회전·전진·후진 후보를 함께 평가해 `/cmd_vel` 발행 |
...
영역이 되도록 비용을 낮춘다. 현재 설정은 Go2의 앞뒤가 긴 footprint에 더해 로컬에서는
`0.45 m`, 전역에서는 `0.50 m`까지 여유 영역을 만든다. LaserScan은 `0.05 m`부터
사용하되, Go2 body footprint 안의 self-hit만 별도 필터로 제거한다.
...
MPPI는 local costmap을 보면서 회전·전진·횡이동·작은 후진 후보를 약 `2초` 앞까지
예상하고, 충돌하지 않으면서 경로와 목표에 잘 맞는 속도를 골라 `/cmd_vel`로 보낸다.
현재 계산 주기는 `20 Hz`다. 목표가 뒤쪽에 있거나 최종 yaw를 맞춰야 할 때에도 별도의
제자리 회전 단계 전환에 의존하지 않는다.

| 파라미터 | 현재 값 | 의미 |
|---|---:|---|
| `controller_frequency` | `20 Hz` | 속도 명령 계산 주기 |
| `max_vel_x` | `0.35 m/s` | 전진 최대 속도 |
| `max_vel_y` | `0.15 m/s` | 횡이동 최대 속도 |
| `wz_max` | `0.35 rad/s` | 회전 최대 속도 |
| `time_steps × model_dt` | `2.0 s` | 각 후보를 미리 예상하는 시간 |
...
| critic | 판단 기준 |
|---|---|
| `ObstaclesCritic` | footprint 기준 충돌·근접 장애물 회피 |
| `PathAlignCritic`, `PathFollowCritic` | 전역 경로 방향과 거리 유지 |
| `GoalCritic`, `GoalAngleCritic` | 목표 위치·방향 접근 |
| `PathAngleCritic` | 진행 방향과 경로 방향의 불일치 완화 |
| `VelocityDeadbandCritic` | Go2가 반응하지 않는 `0.08 rad/s` 미만 yaw 명령 제거 |
...
별도 BT 파일 경로를 고정하지 않고 Nav2 설치본의 기본
...
| 파라미터 | 현재 값 | 의미 |
|---|---:|---|
| `xy_goal_tolerance` | `0.25 m` | 목표 위치 허용 오차 |
| `yaw_goal_tolerance` | `0.20 rad` | 목표 방향 허용 오차 |
| `stateful` | `true` | 위치 판정 후 방향 판정을 유지 |

목표에서 계속 회전하거나 멈추면 goal tolerance, `/go2/odom`, MPPI의 `GoalAngleCritic`, `/cmd_vel` timeout을 함께 확인한다.
...
`linear.z`, `angular.x`, `angular.y`는 이 지상 주행 브리지에서 사용하지 않으므로 0으로 둔다. 현재 Nav2의 최대 출력은 전진 `0.35 m/s`, 횡이동 `0.15 m/s`, 회전 `0.35 rad/s`다.
