...
|---|---|
| 좌·우 스틱 | 로봇 조작에 사용 |
| L1/L2, R1/R2 | 보조 기능 버튼 |
| START, SELECT | 동작 선택·확인 버튼 |
| 전원 버튼·전원 표시등 | 리모컨 전원과 상태 확인 |
| Bluetooth·데이터 전송 표시등 | 로봇과의 연결 및 통신 상태 확인 |
| Type-C 포트·충전 표시등 | 리모컨 충전 |

리모컨을 켜거나 끌 때는 전원 버튼을 짧게 한 번 누른 뒤, 다시 2초 이상 길게 누른다. 켤 때는 알림음이 한 번, 끌 때는 세 번 울린다.
...
`192.168.123.222`는 Go2·Hesai와 통신하는 `eno1`의 유선 주소다. Wi-Fi에 연결한 뒤에는 별도의 Wi-Fi 주소가 할당될 수 있다.

## 6. Wi-Fi 연결과 NoMachine 접속

### Wi-Fi 연결

Jetson AGX의 로컬 화면에서 터미널을 열고 주변 Wi-Fi를 확인한다.
...
현재 Jetson AGX에 할당된 주소를 확인한다.

```bash
hostname -I
```

출력된 주소 중 `192.168.123.222`가 아닌 주소가 Wi-Fi 접속에 사용할 주소다.
...
SSH를 사용할 때도 동일한 Wi-Fi IP 주소를 사용한다.

```bash
ssh ktl@<Jetson_AGX_Wi-Fi_IP>
```

Go2와 Hesai XT16의 통신은 Wi-Fi가 아니라 Jetson AGX의 `eno1` 유선 인터페이스를 사용한다.
...
구현 근거: [go2_state_bridge.cpp](go2_driver/go2_base/src/go2_state_bridge.cpp)
